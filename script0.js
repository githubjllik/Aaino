    ...
    form.innerHTML = `
        ...
        <div id="step1">
            <label>Titre du site (max 30 caractères):
                <input type="text" id="siteTitle" maxlength="30" required>
                <div class="char-counter"><span>0</span>/30</div>
            </label>
            <label>Image du site:
                <input type="file" id="siteImage" accept="image/*" required>
            </label>
        ...
        <div id="step2" class="hidden">
            <label>Description (300-500 caractères):
                <textarea id="siteDescription" minlength="300" maxlength="500" required></textarea>
        ...
        <div id="step5" class="hidden">
            ...
            <button type="button" id="createNewSection">Créer nouvelle section</button>
        </div>
        
        <div class="form-navigation">
            <button type="button" id="prevStep" class="hidden">Précédent</button>
            <button type="button" id="nextStep">Suivant</button>
            <button type="button" id="publishButton" class="hidden">Publier</button>
        </div>
    `;
...
function initContactsHandler() {
    ...
    addButton.onclick = () => {
        if (contactCount >= 4) {
            showErrorMessage('Maximum 4 contacts autorisés');
            return;
        }
        ...
        contactDiv.innerHTML = `
            ...
            <input type="url" placeholder="Lien du contact" required>
            <button type="button" onclick="this.parentElement.remove(); contactCount--;">Supprimer</button>
        `;
...
function validateCurrentStep() {
    ...
    inputs.forEach(input => {
        if (input.required && !input.value) {
            isValid = false;
            showErrorMessage(`Le champ ${input.placeholder || 'requis'} est obligatoire`);
        }
    });
...
async function initSectionHandler() {
    const select = document.getElementById('sectionSelect');
    const createButton = document.getElementById('createNewSection');
    ...
    createButton.onclick = async () => {
        ...
        if (newSectionName) {
            try {
                ...
                showSuccessMessage('Nouvelle section créée avec succès');
            } catch (error) {
                console.error('Erreur lors de la création de la section:', error);
                showErrorMessage('Erreur lors de la création de la section');
            }
        }
    };
}
...
async function publishSite() {
    ...
    try {
        const formData = {
            ...
            tutorial_url: document.getElementById('tutorialUrl').value || null,
        ...
        showErrorMessage('Erreur lors de la publication du site');
    }
}
...
// Ajoutons ces fonctions au code existant
...
async function signOut() {
  try {
    ...
    showErrorMessage('Erreur lors de la déconnexion: ' + error.message);
  }
}
...
async function loadUserProfile() {
  ...
  try {
    ...
    showErrorMessage('Erreur lors du chargement du profil');
  }
}
  ...
  profileDiv.innerHTML = `
    ...
    <div class="user-publications">
      ...
      ${profile.sites.map(site => `
        <div class="publication-item" data-id="${site.id}">
          <h4>${site.title}</h4>
          <p>${site.description}</p>
          <div class="publication-actions">
            <button onclick="editPublication('${site.id}')">Modifier</button>
            <button onclick="confirmDelete('${site.id}')">Supprimer</button>
          </div>
        </div>
      `).join('')}
    </div>
...
async function editPublication(siteId) {
  try {
    ...
    form.innerHTML = `
      ...
      <input type="text" id="edit-title" value="${site.title}" maxlength="30" required>
      <textarea id="edit-description" required>${site.description}</textarea>
      <input type="url" id="edit-url" value="${site.url}" required>
      <input type="url" id="edit-tutorial" value="${site.tutorial_url || ''}">
      <button onclick="saveEdit('${siteId}')">Sauvegarder</button>
    ...
    showErrorMessage('Erreur lors du chargement de la publication');
  }
}
...
async function saveEdit(siteId) {
  try {
    const updates = {
      ...
      tutorial_url: document.getElementById('edit-tutorial').value || null,
      updated_at: new Date()
    };
    ...
    showSuccessMessage('Publication mise à jour avec succès');
    loadUserProfile(); // Recharge le profil pour afficher les changements
  } catch (error) {
...
function confirmDelete(siteId) {
  ...
  modal.innerHTML = `
    <div class="modal-content">
      <h4>Êtes-vous sûr de vouloir supprimer cette publication ?</h4>
      <p>Cette action est irréversible.</p>
      <div class="modal-actions">
        <button onclick="deletePublication('${siteId}')">Confirmer</button>
        <button onclick="this.closest('.confirmation-modal').remove()">Annuler</button>
      </div>
    </div>
  `;
...
async function deletePublication(siteId) {
  try {
    const { error } = await supabase
      .from('sites')
      .delete()
    ...
    showSuccessMessage('Publication supprimée avec succès');
    loadUserProfile(); // Recharge le profil pour afficher les changements
  } catch (error) {
...
// Modification du menu des trois points
menuOptions.push(
  { action: 'edit', icon: 'svg/edit.svg', text: 'Modifier' },
  { action: 'delete', icon: 'svg/delete.svg', text: 'Supprimer' }
);

// Ajout dans le switch pour les nouvelles actions
switch(action) {
  //... autres cases existants
  case 'edit':
    if (isOwner(title)) editPublication(title.dataset.siteId);
    break;
  case 'delete':
    if (isOwner(title)) confirmDelete(title.dataset.siteId);
    break;
}
...
// Modification de la fonction createContextMenu pour n'afficher les options que pour le propriétaire
function createContextMenu(parentElement) {
  const contextMenu = document.createElement('div');
  contextMenu.className = 'context-menu';
  
  menuOptions.forEach(option => {
    if ((option.action === 'edit' || option.action === 'delete') && 
        !isOwner(parentElement)) {
      return; // Skip these options for non-owners
    }
...
// Après la connexion, rediriger vers la dernière page visitée
async function signInWithProvider(provider) {
  try {
